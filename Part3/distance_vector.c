...
{
  int i;
  initrtpkt->source = srcid;
  initrtpkt->dest = destid;
  for (i=0; i<4; i++)
...
main()
{
   ...
   while (1) {
	    ...
	    printf(" src:%2d,",eventptr->rtpktptr->source);
            printf(" dest:%2d,",eventptr->rtpktptr->dest);
  ...
  
{
 ...
 if (packet.source<0 || packet.source >3) {
   printf("WARNING: illegal source id in your packet, ignoring packet!\n");
   return;
   }
 if (packet.dest<0 || packet.dest >3) {
 ...
 if (connectcosts[packet.source][packet.dest] == 999)  {
...
/* to do something with the packet after we return back to him/her */ 
 ...
 mypktptr->source = packet.source;
 mypktptr->dest = packet.dest;
 for (i=0; i<4; i++)
    mypktptr->mincost[i] = packet.mincost[i];
 if (TRACE>2)  {
   printf("    TOLAYER2: source: %d, dest: %d\n              costs:", 
          mypktptr->source, mypktptr->dest);
   for (i=0; i<4; i++)
...
/* create future event for arrival of packet at the other side */
  ...
  evptr->eventity = packet.dest; /* event occurs at other entity */
